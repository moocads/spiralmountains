...
export default function NavLinks() {
  ...
  const isLiveAction = pathname.startsWith("/live-action")
  return (
    <div className="space-y-4 text-xl md:text-[20px] font-bold uppercase">
      {/* Live Action / Works */}
      <Link
  href="/live-action"
  className={`group flex items-center transition-colors hover:text-yellow-400 ${
    pathname === "/live-action" ? "text-yellow-400" : "text-black" 
  }`}
>
  {/* 用三角形替换短线 */}
  <svg
  ...
  className={`group flex items-center transition-colors ${
    pathname === "/contact" ? "text-yellow-400" : "text-black hover:text-yellow-400"
  }`}
>
  <svg
    viewBox="0 0 30 36"
